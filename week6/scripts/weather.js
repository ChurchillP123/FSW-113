...
import toLocaleTemp from './convertTemp.js';
...
const fetchData = async(city) => {
    const response = await fetch(`https://api.openweathermap.org/data/2.5/weather?q=${city}&appid=c9730abdbc43019adc1572bc7128fcc5`);
...
function toLocal(data) {
    temp.innerText = toLocaleTemp(data.main.temp, data);
    humidity.innerText = `${data.main.humidity}%`;
    conditions.innerText = data.weather[0].main;
    document.querySelector('.weatherWrapper').style.backgroundColor = dayOrNight(data);
}