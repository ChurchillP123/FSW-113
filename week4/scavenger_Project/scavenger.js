...
function combineLists() {
    let rajiArr = document.querySelector('#scavenger-0').value.split(',');
    let huangArr = document.querySelector('#scavenger-1').value.split(',');
    let joseArr = document.querySelector('#scavenger-2').value.split(',');
    let veronicaArr = document.querySelector('#scavenger-3').value.split(',');
    let combinedArr = [...rajiArr, ...huangArr, ...joseArr, ...veronicaArr];
    let filterAndSort = combinedArr.filter(elem => elem != "").sort();
    document.querySelector('#AllItems').innerText = filterAndSort.join(', ');
}    
